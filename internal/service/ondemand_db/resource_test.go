...
func TestAccSakuraOnDemandDB_basic(t *testing.T) {
	if test.IsFakeModeEnabled() {
		t.Skip()
	}

	resourceName := "sakura_ondemand_db.foobar"
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraOndemandDBDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraOndemandDB_basic, rand, databaseName, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraOndemandDBExists(resourceName, &reg),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraOndemandDB_update, rand, databaseName, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraOndemandDBExists(resourceName, &reg),
...
func TestAccImportSakuraOndemandDB_basic(t *testing.T) {
	if test.IsFakeModeEnabled() {
		t.Skip()
	}
	...
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             testCheckSakuraOndemandDBDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraOndemandDB_basic, rand, databaseName, password),
			},
...
func testCheckSakuraOndemandDBExists(n string, found *iaas.EnhancedDB) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return errors.New("no OnDemand DB ID is set")
		}

		client := test.AccClientGetter()
		regOp := iaas.NewEnhancedDBOp(client)

		onDemandDB, err := regOp.Read(context.Background(), common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}

		if onDemandDB.ID.String() != rs.Primary.ID {
...
func testCheckSakuraOndemandDBDestroy(s *terraform.State) error {
	client := test.AccClientGetter()

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_ondemand_db" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		...
		if err == nil {
			return errors.New("OnDemand DB still exists")
		}
	}
...
var testAccSakuraOndemandDB_basic = `
...
var testAccSakuraOndemandDB_update = `
resource "sakura_ondemand_db" "foobar" {
  name          = "{{ .arg0 }}-upd"
  database_name = "{{ .arg1 }}"
  database_type = "tidb"
