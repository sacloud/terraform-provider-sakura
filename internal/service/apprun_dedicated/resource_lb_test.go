...
func TestAccSakuraResourceApprunDedicatedLoadBalancer(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraResourceApprunDedicatedLoadBalancer_basic, name, globalClusterID),
				ConfigStateChecks: []statecheck.StateCheck{
					...
					statecheck.ExpectKnownValue(resourceName, tfjsonpath.New("service_class_path"), knownvalue.NotNull()),
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"timeouts", "created", "deleting"},
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					...
					autoScalingGroupID := rs.Primary.Attributes["auto_scaling_group_id"]
					id := rs.Primary.Attributes["id"]
					return fmt.Sprintf("%s/%s/%s", clusterID, autoScalingGroupID, id), nil
				},
			},
		},
	})
...
func testCheckSakuraApprunDedicatedLoadBalancerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		autoScalingGroupID, err := uuid.Parse(rs.Primary.Attributes["auto_scaling_group_id"])
		if err != nil {
			return fmt.Errorf("invalid auto scaling group ID: %s", rs.Primary.Attributes["auto_scaling_group_id"])
		}
		lbID, err := uuid.Parse(rs.Primary.Attributes["id"])
		if err != nil {
			return fmt.Errorf("invalid load balancer ID: %s", rs.Primary.Attributes["id"])
		}
		api := lb.NewLoadBalancerOp(client, v1.ClusterID(clusterID), v1.AutoScalingGroupID(autoScalingGroupID))
		_, err = api.Read(context.Background(), v1.LoadBalancerID(lbID))
		if err == nil {
...
var testAccSakuraResourceApprunDedicatedLoadBalancer_basic = `
locals {
