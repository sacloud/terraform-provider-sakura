...
func (d *clusterDataSource) Read(ctx context.Context, req datasource.ReadRequest, res *datasource.ReadResponse) {
	...
	detail, err := cluster.NewClusterOp(d.client).Read(ctx, *id)
...
func (d *clusterDataSource) byName(ctx context.Context, req datasource.ReadRequest, res *datasource.ReadResponse, state *clusterDataSourceModel) *v1.ClusterID {
	...
	api := cluster.NewClusterOp(d.client)

	list, err := listed(func(c *v1.ClusterID) ([]cluster.ClusterDetail, *v1.ClusterID, error) { return api.List(ctx, 10, c) })
	...
	filtered := slices.Collect(func(yield func(cluster.ClusterDetail) bool) {
		for _, i := range list {
			if i.Name != name {
				continue
			}
	...
	switch len(filtered) {
	case 0:
		res.Diagnostics.AddError("Read: API Error", fmt.Sprintf("failed to read AppRun Dedicated cluster: %s", name))
		return nil

	case 1:
		// filtered[0] is incomplete, read again
		return &filtered[0].ClusterID

	default:
		res.Diagnostics.AddError("Read: API Error", fmt.Sprintf("multiple clusters are named %q", name))
		return nil
	}
}
