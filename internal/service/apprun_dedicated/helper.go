...
func intoInt16[T ~int | ~int32 | ~int16 | ~uint16 | ~int64](t *T) (ret *int16, diag diag.Diagnostics) {
	...
	if rev != *t {
		diag.AddError("integer overflow", fmt.Sprintf("%d cannot be represented as an int16.", *t))
	}
...
func intoUInt16[T ~int | ~int32 | ~int16 | ~int64](t *T) (ret *uint16, diag diag.Diagnostics) {
	...
	if rev != *t {
		diag.AddError("integer overflow", fmt.Sprintf("%d cannot be represented as a uint16.", *t))
	}
