...
func TestAccSakuraDatabase_basicWithWO(t *testing.T) {
	resourceName := "sakura_database.foobar"
	rand := test.RandomName()
	password := test.RandomPassword()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDatabase_basicWithWO, rand, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraDatabaseExists(resourceName, &database),
					testCheckSakuraDatabaseIsMaster(true, &database),
					resource.TestCheckResourceAttr(resourceName, "database_type", "mariadb"),
					...
					resource.TestCheckNoResourceAttr(resourceName, "replica_password"),
					resource.TestCheckNoResourceAttr(resourceName, "replica_password_wo"),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDatabase_updateWithWO, rand, password),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraDatabaseExists(resourceName, &database),
					testCheckSakuraDatabaseIsMaster(true, &database),
					resource.TestCheckResourceAttr(resourceName, "database_type", "mariadb"),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// password, replica_passwordはimport時にstateにセットされないため無視
...
func TestAccImportSakuraDatabase_basicWithWO(t *testing.T) {
	name := test.RandomName()
	password := test.RandomPassword()
	...
	checkFn := func(s []*terraform.InstanceState) error {
		...
		// Note: database_versionのデフォルト値はcomputedなため比較時は無視する
		return test.StateNotEmptyMulti(s[0], "database_version", "network_interface.vswitch_id")
	}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDatabase_importWithWO, name, password),
			},
			{
				ResourceName:      resourceName,
				ImportState:       true,
				ImportStateCheck:  checkFn,
				ImportStateVerify: true,
				// password_wo_versionはimport時にstateにセットされないため無視
				ImportStateVerifyIgnore: []string{
					"password_wo_version",
				},
			},
...
const testAccSakuraDatabase_basicWithWO = `
resource "sakura_vswitch" "foobar" {
  name = "{{ .arg0 }}"
}
...
resource "sakura_database" "foobar" {
  ...
  network_interface = {
    ...
    port          = 33071
    source_ranges = ["192.168.150.0/24", "192.168.151.0/24"]
  }

  name = "{{ .arg0 }}"
}`

const testAccSakuraDatabase_updateWithWO = `
resource "sakura_vswitch" "foobar" {
  name = "{{ .arg0 }}"
}
...
resource "sakura_database" "foobar" {
  ...
  username = "defuser"
  password_wo = "{{ .arg1 }}-upd"
  replica_password_wo = "{{ .arg1 }}-upd"
  ...
  network_interface = {
    ...
    gateway       = "192.168.150.1"
    port          = 33072
    source_ranges = ["192.168.150.0/24", "192.168.161.0/24"]
  }
...
const testAccSakuraDatabase_importWithWO = `
resource "sakura_vswitch" "foobar" {
  name = "{{ .arg0 }}"
}
