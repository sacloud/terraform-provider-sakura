...
func TestAccSakuraResourceDedicatedStorage_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			...
			testCheckSakuraDedicatedStorageDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraResourceDedicatedStorage_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(
						"sakura_dedicated_storage.foobar", "icon_id",
						"sakura_icon.foobar", "id",
					),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraResourceDedicatedStorage_update, rand),
				Check: resource.ComposeTestCheckFunc(
...
func testCheckSakuraDedicatedStorageDestroy(s *terraform.State) error {
	client := test.AccClientGetter()
	triggerOp := eventbus.NewTriggerOp(client.EventBusClient)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_dedicated_storage" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		...
		if err == nil {
			return fmt.Errorf("still exists DedicatedStorage: %s", rs.Primary.ID)
		}
	}
	return nil
}

var testAccSakuraResourceDedicatedStorage_basic = `
...
var testAccSakuraResourceDedicatedStorage_update = `
resource "sakura_dedicated_storage" "foobar" {
  name        = "{{ .arg0 }}-upd"
