// Copyright 2016-2025 The terraform-provider-sakura Authors
// SPDX-License-Identifier: Apache-2.0

package icon_test

import (
	"context"
	"errors"
	"fmt"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/hashicorp/terraform-plugin-testing/terraform"
	"github.com/sacloud/iaas-api-go"
	"github.com/sacloud/terraform-provider-sakura/internal/common"
	"github.com/sacloud/terraform-provider-sakura/internal/test"
)

func TestAccSakuraIcon_basic(t *testing.T) {
	resourceName := "sakura_icon.foobar"
	name := test.RandomName()

	var icon iaas.Icon
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             test.CheckSakuraIconDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraIcon_basic, name),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraIconExists(resourceName, &icon),
					resource.TestCheckResourceAttr(resourceName, "name", name),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraIcon_update, name),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraIconExists(resourceName, &icon),
					resource.TestCheckResourceAttr(resourceName, "name", name+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2-upd"),
				),
			},
		},
	})
}

func TestAccSakuraIcon_withSwitch(t *testing.T) {
	resourceName := "sakura_icon.foobar"
	name := test.RandomName()

	var icon iaas.Icon
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			test.CheckSakuraIconDestroy,
			test.CheckSakuravSwitchDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraIcon_withSwitch, name),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraIconExists(resourceName, &icon),
					resource.TestCheckResourceAttrPair(
						resourceName, "id",
						"sakura_vswitch.foobar", "icon_id",
					),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraIcon_withSwitchUpdate, name),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraIconExists(resourceName, &icon),
					// Frameworkでは設定されていない属性はStateに保存されないため、NoResourceAttrの存在確認テストに変更
					resource.TestCheckNoResourceAttr("sakura_vswitch.foobar", "icon_id"),
				),
			},
		},
	})
}

func TestAccImportSakuraIcon_basic(t *testing.T) {
	resourceName := "sakura_icon.foobar"
	name := test.RandomName()

	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             test.CheckSakuraIconDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraIcon_import, name),
			},
			{
				ResourceName:      resourceName,
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateVerifyIgnore: []string{
					"base64content",
				},
			},
		},
	})
}

func testCheckSakuraIconExists(n string, icon *iaas.Icon) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]

		if !ok {
			return fmt.Errorf("not found: %s", n)
		}

		if rs.Primary.ID == "" {
			return errors.New("no Icon ID is set")
		}

		iconOp := iaas.NewIconOp(test.AccClientGetter())
		foundIcon, err := iconOp.Read(context.Background(), common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}

		if foundIcon.ID.String() != rs.Primary.ID {
			return fmt.Errorf("not found Icon: %s", rs.Primary.ID)
		}
		*icon = *foundIcon

		return nil
	}
}

const testAccSakuraIcon_basic = `
resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  tags          = ["tag1", "tag2"]
  base64content = ""
}`

const testAccSakuraIcon_update = `
resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}-upd"
  tags          = ["tag1-upd", "tag2-upd"]
  base64content = ""
}`

const testAccSakuraIcon_withSwitch = `
resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}

resource "sakura_vswitch" "foobar" {
  name    = "{{ .arg0 }}"
  icon_id = sakura_icon.foobar.id
}
`

const testAccSakuraIcon_withSwitchUpdate = `
resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}

resource "sakura_vswitch" "foobar" {
  name = "{{ .arg0 }}"
}
`

const testAccSakuraIcon_import = `
resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  tags          = ["tag1", "tag2"]
  base64content = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
}
`
