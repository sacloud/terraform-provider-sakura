// Copyright 2016-2025 The terraform-provider-sakura Authors
// SPDX-License-Identifier: Apache-2.0

package archive_test

import (
	"context"
	"errors"
	"fmt"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/hashicorp/terraform-plugin-testing/terraform"
	"github.com/sacloud/iaas-api-go"
	"github.com/sacloud/terraform-provider-sakura/internal/common"
	"github.com/sacloud/terraform-provider-sakura/internal/test"
)

func TestAccSakuraArchive_basic(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	resourceName := "sakura_archive.foobar"
	rand := test.RandomName()

	var archive iaas.Archive
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraArchiveDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraArchive_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraArchiveExists(resourceName, &archive),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "size", "20"),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
					resource.TestCheckResourceAttr(resourceName, "archive_file", "test/dummy.raw"),
					resource.TestCheckResourceAttr(resourceName, "description", "description"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraArchive_update, rand),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "size", "20"),
					resource.TestCheckResourceAttrPair(
						resourceName, "icon_id",
						"sakura_icon.foobar", "id",
					),
					resource.TestCheckResourceAttr(resourceName, "archive_file", "test/dummy-upd.raw"),
					resource.TestCheckResourceAttr(resourceName, "description", "description-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2-upd"),
				),
			},
		},
	})
}

func TestAccSakuraArchive_defaultSize(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	resourceName := "sakura_archive.foobar"
	rand := test.RandomName()

	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraArchiveDestroy,
			test.CheckSakuraDiskDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraArchive_defaultSize, rand),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "size", "40"),
				),
			},
		},
	})
}

func TestAccSakuraArchive_transfer(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	resourceName := "sakura_archive.foobar"
	rand := test.RandomName()

	var archive iaas.Archive
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraArchiveDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraArchive_transfer, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraArchiveExists(resourceName, &archive),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
					resource.TestCheckResourceAttr(resourceName, "description", "description"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
					resource.TestCheckResourceAttrPair(
						resourceName, "source_archive_id",
						"sakura_archive.source", "id",
					),
					resource.TestCheckResourceAttrPair(
						resourceName, "source_archive_zone",
						"sakura_archive.source", "zone",
					),
				),
			},
		},
	})
}

/*
func TestAccSakuraArchive_fromShared(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	resourceName := "sakura_archive.foobar"
	rand := test.RandomName()

	var archive iaas.Archive
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraArchiveDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraArchive_fromShared, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraArchiveExists(resourceName, &archive),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
					resource.TestCheckResourceAttr(resourceName, "description", "description"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
					resource.TestCheckResourceAttrPair(resourceName, "source_shared_key",
						"sakura_archive_share.share", "share_key"),
				),
			},
		},
	})
}
*/

func testCheckSakuraArchiveExists(n string, archive *iaas.Archive) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]

		if !ok {
			return fmt.Errorf("resource %s not found", n)
		}

		if rs.Primary.ID == "" {
			return errors.New("no Archive ID is set")
		}
		client := test.AccClientGetter()
		archiveOp := iaas.NewArchiveOp(client)
		zone := rs.Primary.Attributes["zone"]
		foundArchive, err := archiveOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}
		*archive = *foundArchive
		return nil
	}
}

func testCheckSakuraArchiveDestroy(s *terraform.State) error {
	client := test.AccClientGetter()
	archiveOp := iaas.NewArchiveOp(client)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_archive" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		}

		zone := rs.Primary.Attributes["zone"]
		_, err := archiveOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err == nil {
			return fmt.Errorf("archive[%s] still exists", rs.Primary.ID)
		}
	}

	return nil
}

func TestAccImportSakuraArchive_basic(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	rand := test.RandomName()

	checkFn := func(s []*terraform.InstanceState) error {
		if len(s) != 1 {
			return fmt.Errorf("expected 1 state: %#v", s)
		}

		expects := map[string]string{
			"name": rand,
			"size": "20",
			//"icon_id":      "",
			"archive_file": "",
			"description":  "description",
			"tags.0":       "tag1",
			"tags.1":       "tag2",
		}

		return test.CompareStateMulti(s[0], expects)
	}

	resourceName := "sakura_archive.foobar"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             testCheckSakuraArchiveDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraArchive_basic, rand),
			},
			{
				ResourceName:      resourceName,
				ImportState:       true,
				ImportStateCheck:  checkFn,
				ImportStateVerify: true,
				ImportStateVerifyIgnore: []string{
					"archive_file",
					"hash",
				},
			},
		},
	})
}

var testAccSakuraArchive_basic = `
resource "sakura_archive" "foobar" {
  name        = "{{ .arg0 }}"
  description = "description"
  tags        = ["tag1", "tag2"]

  size         = 20
  archive_file = "test/dummy.raw"
}
`

var testAccSakuraArchive_update = `
resource "sakura_archive" "foobar" {
  name        = "{{ .arg0 }}-upd"
  description = "description-upd"
  tags        = ["tag1-upd" , "tag2-upd"]
  icon_id     = sakura_icon.foobar.id

  size         = 20
  archive_file = "test/dummy-upd.raw"
}

resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}
`

var testAccSakuraArchive_defaultSize = `
resource "sakura_disk" "disk" {
  name = "{{ .arg0 }}"
  size = 40
}

resource "sakura_archive" "foobar" {
  name        = "{{ .arg0 }}"

  source_disk_id = sakura_disk.disk.id
}
`

var testAccSakuraArchive_transfer = `
resource "sakura_archive" "source" {
  name         = "{{ .arg0 }}"
  size         = 20
  archive_file = "test/dummy.raw"

  zone = "is1a"
}

resource "sakura_archive" "foobar" {
  name        = "{{ .arg0 }}"
  description = "description"
  tags        = ["tag1", "tag2"]

  source_archive_zone = sakura_archive.source.zone
  source_archive_id   = sakura_archive.source.id 

  zone = "is1b"
}
`

/*
var testAccSakuraArchive_fromShared = `
resource "sakura_archive" "source" {
  name         = "{{ .arg0 }}"
  size         = 20
  archive_file = "test/dummy.raw"

  zone = "is1a"
}

resource "sakura_archive_share" "share" {
  archive_id = sakura_archive.source.id

  zone = "is1a"
}

resource "sakura_archive" "foobar" {
  name        = "{{ .arg0 }}"
  description = "description"
  tags        = ["tag1", "tag2"]

  source_shared_key = sakura_archive_share.share.share_key

  zone = "is1b"
}
`
*/
