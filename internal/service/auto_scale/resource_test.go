// Copyright 2016-2026 The terraform-provider-sakura Authors
// SPDX-License-Identifier: Apache-2.0

package auto_scale_test

import (
	"context"
	"errors"
	"fmt"
	"os"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/hashicorp/terraform-plugin-testing/terraform"
	"github.com/sacloud/iaas-api-go"
	"github.com/sacloud/terraform-provider-sakura/internal/common"
	"github.com/sacloud/terraform-provider-sakura/internal/test"
)

func TestAccSakuraAutoScale_basic(t *testing.T) {
	resourceName := "sakura_auto_scale.foobar"
	rand := test.RandomName()
	if !test.IsFakeModeEnabled() {
		test.SkipIfEnvIsNotSet(t, "SAKURA_API_KEY_ID")
	}
	apiKeyId := os.Getenv("SAKURA_API_KEY_ID")
	if apiKeyId == "" {
		apiKeyId = "111111111111" // dummy
	}

	var autoScale iaas.AutoScale
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAutoScaleDestroy,
			test.CheckSakuraServerDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoScale_basic, rand, apiKeyId),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAutoScaleExists(resourceName, &autoScale),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "description", "description"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
					resource.TestCheckResourceAttrPair(resourceName, "icon_id", "sakura_icon.foobar", "id"),
					resource.TestCheckResourceAttr(resourceName, "zones.#", "1"),
					resource.TestCheckTypeSetElemAttr(resourceName, "zones.*", "is1c"),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.server_prefix", rand),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.up", "80"),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.down", "20"),
					resource.TestCheckResourceAttr(resourceName, "config", test.BuildConfigWithArgs(testAccSakuraAutoScale_encodedConfig, rand)),
					resource.TestCheckResourceAttrSet(resourceName, "api_key_id"),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoScale_update, rand, apiKeyId),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAutoScaleExists(resourceName, &autoScale),
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "description", "description-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2-upd"),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
					resource.TestCheckResourceAttr(resourceName, "zones.#", "1"),
					resource.TestCheckTypeSetElemAttr(resourceName, "zones.*", "is1c"),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.server_prefix", rand),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.up", "81"),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.down", "21"),
					resource.TestCheckResourceAttr(resourceName, "config", test.BuildConfigWithArgs(testAccSakuraAutoScale_encodedConfig_update, rand)),
					resource.TestCheckResourceAttrSet(resourceName, "api_key_id"),
				),
			},
		},
	})
}

func TestAccSakuraAutoScale_withRouter(t *testing.T) {
	resourceName := "sakura_auto_scale.foobar"
	rand := test.RandomName()
	if !test.IsFakeModeEnabled() {
		test.SkipIfEnvIsNotSet(t, "SAKURA_API_KEY_ID")
	}
	apiKeyId := os.Getenv("SAKURA_API_KEY_ID")
	if apiKeyId == "" {
		apiKeyId = "111111111111" // dummy
	}

	var autoScale iaas.AutoScale
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAutoScaleDestroy,
			test.CheckSakuraInternetDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoScale_withRouter, rand, apiKeyId),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAutoScaleExists(resourceName, &autoScale),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "router_threshold_scaling.router_prefix", rand),
					resource.TestCheckResourceAttr(resourceName, "router_threshold_scaling.direction", "in"),
					resource.TestCheckResourceAttr(resourceName, "router_threshold_scaling.mbps", "20"),
					resource.TestCheckResourceAttr(resourceName, "config", test.BuildConfigWithArgs(testAccSakuraAutoScale_encodedConfigWithRouter, rand)),
					resource.TestCheckResourceAttrSet(resourceName, "api_key_id"),
				),
			},
		},
	})
}

func TestAccSakuraAutoScale_withScheduleTrigger(t *testing.T) {
	resourceName := "sakura_auto_scale.foobar"
	rand := test.RandomName()
	if !test.IsFakeModeEnabled() {
		test.SkipIfEnvIsNotSet(t, "SAKURA_API_KEY_ID")
	}
	apiKeyId := os.Getenv("SAKURA_API_KEY_ID")
	if apiKeyId == "" {
		apiKeyId = "111111111111" // dummy
	}

	var autoScale iaas.AutoScale
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAutoScaleDestroy,
			test.CheckSakuraInternetDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoScale_withScheduleTrigger, rand, apiKeyId),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAutoScaleExists(resourceName, &autoScale),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "cpu_threshold_scaling.#", "0"),
					resource.TestCheckResourceAttr(resourceName, "router_threshold_scaling.#", "0"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.0.action", "up"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.0.hour", "10"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.0.minute", "15"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.0.days_of_week.#", "2"),
					resource.TestCheckTypeSetElemAttr(resourceName, "schedule_scaling.0.days_of_week.*", "mon"),
					resource.TestCheckTypeSetElemAttr(resourceName, "schedule_scaling.0.days_of_week.*", "tue"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.1.hour", "11"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.1.minute", "30"),
					resource.TestCheckResourceAttr(resourceName, "schedule_scaling.1.days_of_week.#", "2"),
					resource.TestCheckTypeSetElemAttr(resourceName, "schedule_scaling.1.days_of_week.*", "sat"),
					resource.TestCheckTypeSetElemAttr(resourceName, "schedule_scaling.1.days_of_week.*", "sun"),
					resource.TestCheckResourceAttrSet(resourceName, "api_key_id"),
				),
			},
		},
	})
}

func testCheckSakuraAutoScaleExists(n string, auto_scale *iaas.AutoScale) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]

		if !ok {
			return fmt.Errorf("not found: %s", n)
		}

		if rs.Primary.ID == "" {
			return errors.New("no AutoScale ID is set")
		}

		client := test.AccClientGetter()
		autoScaleOp := iaas.NewAutoScaleOp(client)

		foundAutoScale, err := autoScaleOp.Read(context.Background(), common.SakuraCloudID(rs.Primary.ID))

		if err != nil {
			return err
		}

		if foundAutoScale.ID.String() != rs.Primary.ID {
			return errors.New("resource not found")
		}

		*auto_scale = *foundAutoScale

		return nil
	}
}

func testCheckSakuraAutoScaleDestroy(s *terraform.State) error {
	client := test.AccClientGetter()

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_auto_scale" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		}

		autoScaleOp := iaas.NewAutoScaleOp(client)
		_, err := autoScaleOp.Read(context.Background(), common.SakuraCloudID(rs.Primary.ID))

		if err == nil {
			return errors.New("AutoScale still exists")
		}
	}

	return nil
}

func TestAccImportSakuraAutoScale_basic(t *testing.T) {
	if !test.IsFakeModeEnabled() {
		test.SkipIfEnvIsNotSet(t, "SAKURA_API_KEY_ID")
	}
	apiKeyId := os.Getenv("SAKURA_API_KEY_ID")
	rand := test.RandomName()

	checkFn := func(s []*terraform.InstanceState) error {
		if len(s) != 1 {
			return fmt.Errorf("expected 1 state: %#v", s)
		}
		expects := map[string]string{
			"name":                                rand,
			"description":                         "description",
			"tags.0":                              "tag1",
			"tags.1":                              "tag2",
			"zones.0":                             "is1c",
			"api_key_id":                          apiKeyId,
			"cpu_threshold_scaling.server_prefix": rand,
			"cpu_threshold_scaling.up":            "80",
			"cpu_threshold_scaling.down":          "20",
			"config":                              test.BuildConfigWithArgs(testAccSakuraAutoScale_encodedConfig, rand),
		}

		if err := test.CompareStateMulti(s[0], expects); err != nil {
			return err
		}
		return test.StateNotEmptyMulti(s[0], "icon_id")
	}

	resourceName := "sakura_auto_scale.foobar"

	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAutoScaleDestroy,
			test.CheckSakuraServerDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoScale_basic, rand, os.Getenv("SAKURA_API_KEY_ID")),
			},
			{
				ResourceName:      resourceName,
				ImportState:       true,
				ImportStateCheck:  checkFn,
				ImportStateVerify: true,
			},
		},
	})
}

var testAccSakuraAutoScale_basic = `
resource "sakura_server" "foobar" {
  name = "{{ .arg0 }}"
  force_shutdown = true
  zone = "is1c"
}

resource "sakura_auto_scale" "foobar" {
  name        = "{{ .arg0 }}"
  description = "description"
  tags        = ["tag1", "tag2"]
  icon_id     = sakura_icon.foobar.id
  zones       = ["is1c"]

  config = yamlencode({
    resources: [{
      type: "Server",
      selector: {
        names: [sakura_server.foobar.name],
        zones: ["is1c"],
      },
      shutdown_force: true,
    }],
  })

  api_key_id = "{{ .arg1 }}"

  trigger_type = "cpu"
  cpu_threshold_scaling = {
    server_prefix = "{{ .arg0 }}"
    up   = 80
    down = 20
  }
}

resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}
`

var testAccSakuraAutoScale_encodedConfig = `"resources":
- "selector":
    "names":
    - "{{ .arg0 }}"
    "zones":
    - "is1c"
  "shutdown_force": true
  "type": "Server"
`

var testAccSakuraAutoScale_update = `
resource "sakura_server" "foobar" {
  name = "{{ .arg0 }}"
  force_shutdown = true
  zone = "is1c"
}

resource "sakura_auto_scale" "foobar" {
  name        = "{{ .arg0 }}-upd"
  description = "description-upd"
  tags        = ["tag1-upd", "tag2-upd"]
  zones       = ["is1c"]
  
  config = yamlencode({
    resources: [{
      type: "Server",
      selector: {
        names: [sakura_server.foobar.name],
        zones: ["is1c"],
      },
      shutdown_force: true,
    }],
    autoscaler: {
      cooldown: {
        down: 500,
        up: 300,
      }
    },
  })

  api_key_id = "{{ .arg1 }}"

  trigger_type = "cpu"
  cpu_threshold_scaling = {
    server_prefix = "{{ .arg0 }}"
    up   = 81
    down = 21
  }
}
`

var testAccSakuraAutoScale_encodedConfig_update = `"autoscaler":
  "cooldown":
    "down": 500
    "up": 300
"resources":
- "selector":
    "names":
    - "{{ .arg0 }}"
    "zones":
    - "is1c"
  "shutdown_force": true
  "type": "Server"
`

var testAccSakuraAutoScale_withRouter = `
resource "sakura_internet" "foobar" {
  name = "{{ .arg0 }}"
  zone = "is1c"
}

resource "sakura_auto_scale" "foobar" {
  name   = "{{ .arg0 }}"
  zones  = ["is1c"]
  config = yamlencode({
    resources: [{
      type: "Router",
      selector: {
        names: [sakura_internet.foobar.name],
        zones: ["is1c"],
      },
    }],
  })

  api_key_id = "{{ .arg1 }}"

  trigger_type = "router"
  router_threshold_scaling = {
    router_prefix = "{{ .arg0 }}"
    direction     = "in"
    mbps          = 20
  }
}
`

var testAccSakuraAutoScale_encodedConfigWithRouter = `"resources":
- "selector":
    "names":
    - "{{ .arg0 }}"
    "zones":
    - "is1c"
  "type": "Router"
`

var testAccSakuraAutoScale_withScheduleTrigger = `
resource "sakura_server" "foobar" {
  name = "{{ .arg0 }}"
  force_shutdown = true
  zone = "is1c"
}

resource "sakura_auto_scale" "foobar" {
  name   = "{{ .arg0 }}"
  zones  = ["is1c"]

  config = yamlencode({
    resources: [{
      type: "Server",
      selector: {
        names: [sakura_server.foobar.name],
        zones: ["is1c"],
      },
      shutdown_force: true,
    }],
  })

  api_key_id = "{{ .arg1 }}"

  trigger_type = "schedule"
  schedule_scaling = [{
    action = "up"
    hour   = 10
    minute = 15
    days_of_week = ["mon", "tue"]
  },
  {
    action = "down"
    hour   = 11
    minute = 30
    days_of_week = ["sat", "sun"]
  }]
}
`
