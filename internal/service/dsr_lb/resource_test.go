		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "network_interface.ip_addresses.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "network_interface.ip_addresses.0", "192.168.11.101"),
...
func testCheckSakuraDSRLBExists(n string, loadBalancer *iaas.LoadBalancer) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		foundLoadBalancer, err := lbOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}
...
func TestAccImportSakuraDSRLB_basic(t *testing.T) {
	...
	checkFn := func(s []*terraform.InstanceState) error {
		...
		expects := map[string]string{
			...
			"network_interface.ip_addresses.0": "192.168.11.101",
			"network_interface.netmask":        "24",
			"network_interface.gateway":        "192.168.11.1",
...
resource "sakura_dsr_lb" "foobar" {
  ...
  network_interface = {
    ...
    vrid         = 1
    ip_addresses = [sakura_internet.foobar.ip_addresses[0], sakura_internet.foobar.ip_addresses[1]]
