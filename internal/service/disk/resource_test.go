...
func TestAccSakuraDisk_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDisk_basic, rand),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDisk_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraDiskExists(resourceName, &disk),
					testCheckSakuraDiskAttributes(&disk),
...
func TestAccSakuraDisk_with_Server(t *testing.T) {
	test.SkipIfFakeModeEnabled(t) // FakeModeだとip_address指定が動かないためスキップする

	diskResourceName := "sakura_disk.foobar"
	serverResourceName := "sakura_server.foobar"
	rand := test.RandomName()
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDisk_with_Server, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraDiskExists(diskResourceName, &disk),
					testCheckSakuraDiskAttributes(&disk),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDisk_with_Server_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraDiskExists(diskResourceName, &disk),
					testCheckSakuraDiskAttributes(&disk),
...
func testCheckSakuraDiskExists(n string, disk *iaas.Disk) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		foundDisk, err := diskOp.Read(ctx, zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}
...
func testCheckSakuraDiskAttributes(disk *iaas.Disk) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func TestAccImportSakuraDisk_basic(t *testing.T) {
	...
	checkFn := func(s []*terraform.InstanceState) error {
		...
		return test.StateNotEmptyMulti(s[0], "source_archive_id", "kms_key_id", "icon_id")
	}
...
var testAccSakuraDisk_with_Server = `
data "sakura_archive" "ubuntu" {
  os_type = "ubuntu"
}
...
var testAccSakuraDisk_with_Server_update = `
data "sakura_archive" "ubuntu" {
  os_type = "ubuntu"
}
