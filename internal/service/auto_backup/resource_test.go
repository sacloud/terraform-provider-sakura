// Copyright 2016-2025 The terraform-provider-sakura Authors
// SPDX-License-Identifier: Apache-2.0

package auto_backup_test

import (
	"context"
	"errors"
	"fmt"
	"os"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/hashicorp/terraform-plugin-testing/terraform"
	"github.com/sacloud/iaas-api-go"
	"github.com/sacloud/terraform-provider-sakura/internal/common"
	"github.com/sacloud/terraform-provider-sakura/internal/test"
)

func TestAccSakuraAutoBackup_basic(t *testing.T) {
	resourceName := "sakura_auto_backup.foobar"
	rand := test.RandomName()

	var autoBackup iaas.AutoBackup
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAutoBackupDestroy,
			test.CheckSakuraDiskDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoBackup_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAutoBackupExists(resourceName, &autoBackup),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "days_of_week.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "days_of_week.0", "fri"),
					resource.TestCheckResourceAttr(resourceName, "days_of_week.1", "wed"),
					resource.TestCheckResourceAttr(resourceName, "max_backup_num", "1"),
					resource.TestCheckResourceAttr(resourceName, "description", "description"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
					resource.TestCheckResourceAttrPair(
						resourceName, "icon_id",
						"sakura_icon.foobar", "id",
					),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoBackup_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAutoBackupExists(resourceName, &autoBackup),
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "days_of_week.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "days_of_week.0", "fri"),
					resource.TestCheckResourceAttr(resourceName, "days_of_week.1", "sun"),
					resource.TestCheckResourceAttr(resourceName, "max_backup_num", "2"),
					resource.TestCheckResourceAttr(resourceName, "description", "description-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2-upd"),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
				),
			},
		},
	})
}

func testCheckSakuraAutoBackupExists(n string, auto_backup *iaas.AutoBackup) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("not found: %s", n)
		}
		if rs.Primary.ID == "" {
			return errors.New("no AutoBackup ID is set")
		}

		client := test.AccClientGetter()
		zone := rs.Primary.Attributes["zone"]
		found, err := iaas.NewAutoBackupOp(client).Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}

		if found.ID.String() != rs.Primary.ID {
			return errors.New("resource not found")
		}

		*auto_backup = *found
		return nil
	}
}

func testCheckSakuraAutoBackupDestroy(s *terraform.State) error {
	client := test.AccClientGetter()

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_auto_backup" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		}

		zone := rs.Primary.Attributes["zone"]
		_, err := iaas.NewAutoBackupOp(client).Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err == nil {
			return errors.New("AutoBackup still exists")
		}
	}

	return nil
}

func TestAccImportSakuraAutoBackup_basic(t *testing.T) {
	rand := test.RandomName()

	checkFn := func(s []*terraform.InstanceState) error {
		if len(s) != 1 {
			return fmt.Errorf("expected 1 state: %#v", s)
		}
		expects := map[string]string{
			"name":           rand,
			"days_of_week.0": "fri",
			"days_of_week.1": "wed",
			"max_backup_num": "1",
			"description":    "description",
			"tags.0":         "tag1",
			"tags.1":         "tag2",
			"zone":           os.Getenv("SAKURA_ZONE"),
		}

		if err := test.CompareStateMulti(s[0], expects); err != nil {
			return err
		}
		return test.StateNotEmptyMulti(s[0], "disk_id", "icon_id")
	}

	resourceName := "sakura_auto_backup.foobar"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAutoBackupDestroy,
			test.CheckSakuraDiskDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAutoBackup_basic, rand),
			},
			{
				ResourceName:      resourceName,
				ImportState:       true,
				ImportStateCheck:  checkFn,
				ImportStateVerify: true,
			},
		},
	})
}

var testAccSakuraAutoBackup_basic = `
resource "sakura_disk" "foobar" {
  name = "{{ .arg0 }}"
}

resource "sakura_auto_backup" "foobar" {
  name           = "{{ .arg0 }}"
  disk_id        = sakura_disk.foobar.id
  days_of_week   = ["wed", "fri"]
  max_backup_num = 1
  description    = "description"
  tags           = ["tag1", "tag2"]
  icon_id        = sakura_icon.foobar.id
}

resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}
`

var testAccSakuraAutoBackup_update = `
resource "sakura_disk" "foobar" {
  name = "{{ .arg0 }}"
}

resource "sakura_auto_backup" "foobar" {
  name           = "{{ .arg0 }}-upd"
  disk_id        = sakura_disk.foobar.id
  days_of_week   = ["sun", "fri"]
  max_backup_num = 2
  description    = "description-upd"
  tags           = ["tag1-upd", "tag2-upd"]
}`
