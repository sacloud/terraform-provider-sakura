...
func TestAccSakuraVPNRouter_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraVPNRouter_basic, rand),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraVPNRouter_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraVPNRouterExists(resourceName, &vpcRouter),
...
func TestAccSakuraVPNRouter_Full(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraVPNRouter_complete, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraVPNRouterExists(resourceName, &vpcRouter),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "version", "2"),
					...
					resource.TestCheckResourceAttr(resourceName, "dns_forwarding.interface_index", "1"),
					resource.TestCheckResourceAttr(resourceName, "dns_forwarding.dns_servers.#", "2"),
					...
					resource.TestCheckResourceAttr(resourceName, "wire_guard.ip_address", "192.168.31.1/24"),
					resource.TestCheckResourceAttrSet(resourceName, "wire_guard.public_key"),
					resource.TestCheckResourceAttr(resourceName, "wire_guard.peer.0.name", "example"),
					resource.TestCheckResourceAttr(resourceName, "wire_guard.peer.0.ip_address", "192.168.31.11"),
					resource.TestCheckResourceAttr(resourceName, "wire_guard.peer.0.public_key", "fqxOlS2X0Jtg4P9zVf8D3BAUtJmrp+z2mjzUmgxxxxx="),
					...
					resource.TestCheckResourceAttrPair(
						...
						"sakura_internet.foobar", "ip_addresses.3"),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraVPNRouter_completeUpdate, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraVPNRouterExists(resourceName, &vpcRouter),
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttrSet(resourceName, "public_ip"),
...
func testCheckSakuraVPNRouterExists(n string, vpcRouter *iaas.VPCRouter) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		foundVPCRouter, err := vrOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}
...
func testCheckSakuraVPNRouterDestroy(s *terraform.State) error {
	client := test.AccClientGetter()
	vrOp := iaas.NewVPCRouterOp(client)
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_vpn_router" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		...
		zone := rs.Primary.Attributes["zone"]
		_, err := vrOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
...
var testAccSakuraVPNRouter_basic = `
...
var testAccSakuraVPNRouter_update = `
resource "sakura_vpn_router" "foobar" {
  name                = "{{ .arg0 }}-upd"
...
var testAccSakuraVPNRouter_complete = `
...
var testAccSakuraVPNRouter_completeUpdate = `
resource "sakura_internet" "foobar" {
  name = "{{ .arg0 }}"
}
resource "sakura_vswitch" "foobar" {
