// Copyright 2016-2025 The terraform-provider-sakura Authors
// SPDX-License-Identifier: Apache-2.0

package script_test

import (
	"context"
	"errors"
	"fmt"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/hashicorp/terraform-plugin-testing/terraform"
	"github.com/sacloud/iaas-api-go"
	"github.com/sacloud/terraform-provider-sakura/internal/common"
	"github.com/sacloud/terraform-provider-sakura/internal/test"
)

func TestAccSakuraScript_basic(t *testing.T) {
	resourceName := "sakura_script.foobar"
	rand := test.RandomName()

	var note iaas.Note
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             testCheckSakuraCloudNoteDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraScript_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraScriptExists(resourceName, &note),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "content", "content"),
					resource.TestCheckResourceAttr(resourceName, "class", "shell"),
					resource.TestCheckResourceAttrPair(
						resourceName, "icon_id",
						"sakura_icon.foobar", "id",
					),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraScript_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraScriptExists(resourceName, &note),
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "content", "content-upd"),
					resource.TestCheckResourceAttr(resourceName, "class", "shell"),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
				),
			},
		},
	})
}

func TestAccSakuraScript_withYAML(t *testing.T) {
	resourceName := "sakura_script.foobar"
	name := test.RandomName()

	var note iaas.Note
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             testCheckSakuraCloudNoteDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraScript_yaml, name),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraScriptExists(resourceName, &note),
					resource.TestCheckResourceAttr(resourceName, "name", name),
					resource.TestCheckResourceAttr(resourceName, "content", "#cloud_init"),
					resource.TestCheckResourceAttr(resourceName, "class", "yaml_cloud_config"),
				),
			},
		},
	})
}

func testCheckSakuraScriptExists(n string, note *iaas.Note) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]

		if !ok {
			return fmt.Errorf("not found: %s", n)
		}

		if rs.Primary.ID == "" {
			return errors.New("no Note ID is set")
		}

		client := test.AccClientGetter()
		noteOp := iaas.NewNoteOp(client)

		foundNote, err := noteOp.Read(context.Background(), common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}

		if foundNote.ID.String() != rs.Primary.ID {
			return fmt.Errorf("not found Note: %s", rs.Primary.ID)
		}

		*note = *foundNote
		return nil
	}
}

func testCheckSakuraCloudNoteDestroy(s *terraform.State) error {
	client := test.AccClientGetter()
	noteOp := iaas.NewNoteOp(client)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_script" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		}

		_, err := noteOp.Read(context.Background(), common.SakuraCloudID(rs.Primary.ID))
		if err == nil {
			return fmt.Errorf("still exists Note: %s", rs.Primary.ID)
		}
	}

	return nil
}

const testAccSakuraScript_basic = `
resource "sakura_script" "foobar" {
  name    = "{{ .arg0 }}"
  content = "content"
  icon_id = sakura_icon.foobar.id
}

resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}
`

const testAccSakuraScript_update = `
resource "sakura_script" "foobar" {
  name    = "{{ .arg0 }}-upd"
  content = "content-upd"
}`

const testAccSakuraScript_yaml = `
resource "sakura_script" "foobar" {
  name    = "{{ .arg0 }}"
  content = "#cloud_init"
  class   = "yaml_cloud_config"
}`
