// Copyright 2016-2025 The terraform-provider-sakura Authors
// SPDX-License-Identifier: Apache-2.0

package cdrom_test

import (
	"context"
	"errors"
	"fmt"
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/resource"
	"github.com/hashicorp/terraform-plugin-testing/terraform"
	"github.com/sacloud/iaas-api-go"
	"github.com/sacloud/terraform-provider-sakura/internal/common"
	"github.com/sacloud/terraform-provider-sakura/internal/test"
)

func TestAccSakuraCDROM_basic(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	resourceName := "sakura_cdrom.foobar"
	rand := test.RandomName()

	var cdrom iaas.CDROM
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraCDROMDestroy,
			test.CheckSakuraIconDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraCDROM_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraCDROMExists(resourceName, &cdrom),
					resource.TestCheckResourceAttr(resourceName, "name", rand),
					resource.TestCheckResourceAttr(resourceName, "size", "5"),
					resource.TestCheckNoResourceAttr(resourceName, "icon_id"),
					resource.TestCheckResourceAttr(resourceName, "iso_image_file", "test/dummy.iso"),
					resource.TestCheckResourceAttr(resourceName, "description", "description"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2"),
				),
			},
			{
				Config: test.BuildConfigWithArgs(testAccSakuraCDROM_update, rand),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", rand+"-upd"),
					resource.TestCheckResourceAttr(resourceName, "size", "5"),
					resource.TestCheckResourceAttrPair(
						resourceName, "icon_id",
						"sakura_icon.foobar", "id",
					),
					resource.TestCheckResourceAttr(resourceName, "iso_image_file", "test/dummy-upd.iso"),
					resource.TestCheckResourceAttr(resourceName, "description", "description-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.0", "tag1-upd"),
					resource.TestCheckResourceAttr(resourceName, "tags.1", "tag2-upd"),
				),
			},
		},
	})
}

func testCheckSakuraCDROMExists(n string, cdrom *iaas.CDROM) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]

		if !ok {
			return fmt.Errorf("resource %s not found", n)
		}

		if rs.Primary.ID == "" {
			return errors.New("no CD-ROM ID is set")
		}
		client := test.AccClientGetter()
		cdromOp := iaas.NewCDROMOp(client)
		zone := rs.Primary.Attributes["zone"]
		found, err := cdromOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}
		*cdrom = *found
		return nil
	}
}

func testCheckSakuraCDROMDestroy(s *terraform.State) error {
	client := test.AccClientGetter()
	cdromOp := iaas.NewCDROMOp(client)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_cdrom" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		}

		zone := rs.Primary.Attributes["zone"]
		_, err := cdromOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err == nil {
			return fmt.Errorf("cdrom[%s] still exists", rs.Primary.ID)
		}
	}

	return nil
}

func TestAccImportSakuraCDROM_basic(t *testing.T) {
	test.SkipIfFakeModeEnabled(t)

	rand := test.RandomName()

	checkFn := func(s []*terraform.InstanceState) error {
		if len(s) != 1 {
			return fmt.Errorf("expected 1 state: %#v", s)
		}

		expects := map[string]string{
			"name":           rand,
			"size":           "5",
			"iso_image_file": "",
			"description":    "description",
			"tags.0":         "tag1",
			"tags.1":         "tag2",
		}

		return test.CompareStateMulti(s[0], expects)
	}

	resourceName := "sakura_cdrom.foobar"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { test.AccPreCheck(t) },
		ProtoV6ProviderFactories: test.AccProtoV6ProviderFactories,
		CheckDestroy:             testCheckSakuraCDROMDestroy,
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraCDROM_basic, rand),
			},
			{
				ResourceName:      resourceName,
				ImportState:       true,
				ImportStateCheck:  checkFn,
				ImportStateVerify: true,
				ImportStateVerifyIgnore: []string{
					"iso_image_file",
					"hash",
				},
			},
		},
	})
}

var testAccSakuraCDROM_basic = `
resource "sakura_cdrom" "foobar" {
  name        = "{{ .arg0 }}"
  description = "description"
  tags        = ["tag1", "tag2"]

  size           = 5
  iso_image_file = "test/dummy.iso"
}
`

var testAccSakuraCDROM_update = `
resource "sakura_cdrom" "foobar" {
  name        = "{{ .arg0 }}-upd"
  description = "description-upd"
  tags        = ["tag1-upd" , "tag2-upd"]
  icon_id     = sakura_icon.foobar.id

  size           = 5
  iso_image_file = "test/dummy-upd.iso"
}

resource "sakura_icon" "foobar" {
  name          = "{{ .arg0 }}"
  base64content = ""
}
`
