...
func TestAccSakuraNFS_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraNFS_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraNFSExists(resourceName, &nfs),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraNFS_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraNFSExists(resourceName, &nfs),
...
func testCheckSakuraNFSExists(n string, nfs *iaas.NFS) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		foundNFS, err := nfsOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}
...
func testCheckSakuraNFSDestroy(s *terraform.State) error {
	client := test.AccClientGetter()
	nfsOp := iaas.NewNFSOp(client)
