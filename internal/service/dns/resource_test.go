...
func TestAccSakuraDNS_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDNS_basic, zone),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraDNS_update, zone),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraDNSExists(resourceName, &dns),
					resource.TestCheckResourceAttr(resourceName, "zone", zone),
...
func testCheckSakuraDNSExists(n string, dns *iaas.DNS) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		foundDNS, err := dnsOp.Read(ctx, common.SakuraCloudID(rs.Primary.ID))
