...
func TestAccSakuraAddonCDN_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeTestCheckFunc(
			testCheckSakuraAddonCDNDestroy,
		),
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraAddonCDN_basic),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraAddonCDNExists(resourceName, &cdn),
...
func testCheckSakuraAddonCDNExists(n string, res *v1.GetResourceResponse) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return errors.New("no Addon CDN ID is set")
		}

		client := test.AccClientGetter()
		found, err := addon.NewCDNOp(client.AddonClient).Read(context.Background(), rs.Primary.ID)
		if err != nil {
			return err
		}
...
func testCheckSakuraAddonCDNDestroy(s *terraform.State) error {
	client := test.AccClientGetter()

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "sakura_addon_cdn" {
			continue
		}
		if rs.Primary.ID == "" {
			continue
		...
		_, err := addon.NewCDNOp(client.AddonClient).Read(context.Background(), rs.Primary.ID)
		if err == nil {
			return errors.New("Addon CDN still exists")
		}
	}
...
var testAccSakuraAddonCDN_basic = `
