...
func TestAccSakuravSwitch_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: test.BuildConfigWithArgs(testAccSakuraSwitch_basic, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraSwitchExists(resourceName, &sw),
			...
			{
				Config: test.BuildConfigWithArgs(testAccSakuraSwitch_update, rand),
				Check: resource.ComposeTestCheckFunc(
					testCheckSakuraSwitchExists(resourceName, &sw),
...
func testCheckSakuraSwitchExists(n string, sw *iaas.Switch) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return errors.New("no vSwitch ID is set")
		}

		swOp := iaas.NewSwitchOp(test.AccClientGetter())
		zone := rs.Primary.Attributes["zone"]

		foundSwitch, err := swOp.Read(context.Background(), zone, common.SakuraCloudID(rs.Primary.ID))
		if err != nil {
			return err
		}

		if foundSwitch.ID.String() != rs.Primary.ID {
			return fmt.Errorf("resource vSwitch[%s] is not found", rs.Primary.ID)
		}
...
var testAccSakuraSwitch_update = `
