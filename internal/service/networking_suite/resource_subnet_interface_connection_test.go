...
func TestAccSakuraNetworkingSuiteSubnetInterfaceConnection_basic(t *testing.T) {
	test.SkipIfEnvIsNotSet(t, "SAKURA_NETWORKING_SUITE_REGION", "SAKURA_ZONE", "SAKURA_NETWORKING_SUITE_INTERFACE_SRN")

	resourceName := "sakura_networking_suite_subnet_interface_connection.foobar"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "settings.enable_ip_forwarding", "false"),
				),
			},
		},
	})
...
resource "sakura_networking_suite_subnet_interface_connection" "foobar" {
